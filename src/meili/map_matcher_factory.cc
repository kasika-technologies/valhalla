...
Config MapMatcherFactory::MergeConfig(const Options& options) const {
  ...
  if (options.optional_search_radius_case() && config.candidate_search.is_search_radius_customizable) {
    config.candidate_search.search_radius_meters = options.search_radius();
  }
  if (options.optional_turn_penalty_factor_case() &&
  ...
  if (options.optional_gps_accuracy_case() && config.emission_cost.is_gps_accuracy_customizable) {
    config.emission_cost.gps_accuracy_meters = options.gps_accuracy();
  }
  if (options.optional_breakage_distance_case() && config.transition_cost.is_breakage_distance_customizable) {
    config.transition_cost.breakage_distance_meters = options.breakage_distance();
  }
  if (options.optional_interpolation_distance_case() && config.routing.is_interpolation_distance_customizable) {
