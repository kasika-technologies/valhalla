...
void loki_worker_t::init_trace(Api& request) {
  ...
  float breakage_distance =
      options.optional_breakage_distance_case() ? options.breakage_distance() : default_breakage_distance;
  ...
  if (options.shape_match() == ShapeMatch::map_snap) {
    check_best_paths(options.alternates() + 1, max_best_paths);
    check_best_paths_shape(options.alternates() + 1, options.shape(), max_best_paths_shape);
  }

  // Validate optional trace options
  if (options.optional_gps_accuracy_case()) {
    check_gps_accuracy(options.gps_accuracy(), max_gps_accuracy);
  }
  if (options.optional_search_radius_case()) {
